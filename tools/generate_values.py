...
directory_to_scan = '../modules'  # Directory containing the configuration files
output_values_path = '../helm/alloy-modules/configmaps/values.yaml'  # Path to the values.yaml file in your Helm chart

files_list = []
for subdir, dirs, files in os.walk(directory_to_scan):
    for file in files:
        filepath = os.path.join(subdir, file)
        if filepath.endswith(".alloy"):
