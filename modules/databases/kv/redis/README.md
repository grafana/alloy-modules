...
Handles discovery of kubernetes targets and exports them, this component does not perform any scraping at all and is not required to be used for kubernetes, as a custom service discovery and targets can be defined and passed to `redis.scrape`
...
| :----- | :------- | :------ | :------------------------------------- |
| `port` | `true`   | `9121`  | The of the port to scrape metrics from |
...
| `job_label`       | _no_     | `integrations/redis`          | The job label to add for all mimir metric                                                                                                           |
...
import.git "redis" {
  ...
  revision   = "main"
  path       = "v2/integrations/redis/module.river"
  pull_frequency = "15m"
}

// get the targets
redis.kubernetes "targets" {}

// scrape the targets
redis.scrape "metrics" {
  targets = redis.kubernetes.targets.output
  forward_to = [
...
// scrape the targets
redis.scrape "metrics" {
  targets = redis.local.targets.output
  forward_to = [
    prometheus.remote_write.default.receiver,
  ]
}
